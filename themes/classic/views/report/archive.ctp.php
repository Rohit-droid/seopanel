...
<?php } else {?>
	...
	<form id='search_form'>
	<table width="100%" class="search" id="custom-dashboard-heading-rr">
		<tr>
			<td style="flex:32;">
				<input type="text" placeholder="Name" name="search_name" value="<?php echo htmlentities($searchInfo['search_name'], ENT_QUOTES)?>" onblur="<?php echo $submitLink?>">
			</td>
    		<td style="order:1;flex:32; display:flex;">
    			<input type="text" class="calender-icon-RR" style="margin-right:20px;" value="<?php echo $fromTime?>" name="from_time" id="from_time"/>
    			<input type="text"  class="calender-icon-RR" value="<?php echo $toTime?>" name="to_time" id="to_time"/>
				<script>
				  $( function() {
			...
			<td style="flex:22;">
    			<select name="website_id" id="website_id"  onchange="scriptDoLoadPost('archive.php', 'search_form', 'content')">
			...
			<td style="flex:22;">
				<select name="report_type" id="report_type" onchange="scriptDoLoadPost('archive.php', 'search_form', 'content')">
					<option value="">-- <?php echo $spText['common']['Select']?> --</option>
...
<div id='subcontent' class="dashboard" style="margin-top:0px;">
<?php
    	...
    	<table id="cust_tab">
    		...
    		if(count($websiteRankList) > 0){
				...
				foreach($websiteRankList as $websiteInfo){
    				...
    				<tr>
    					<td id="cts-link">
    						<a style="color:#0033A0" href="javascript:void(0)"><?php echo $websiteInfo['url'];?></a>
    					</td>
