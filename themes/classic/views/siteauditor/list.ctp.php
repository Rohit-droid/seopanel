...
<?php if(!empty($isAdmin)){ ?>
	<table border="0" cellspacing="0" cellpadding="0" class="search" style="width:20%;">
		<tr>
			<td>
...
<?php } ?>
<?php echo $pagingDiv?>
<table width="100%" border="0" cellspacing="0" cellpadding="0" class="list auditor-Page-RR" style="margin:50px auto;">
	<tr class="listHead">
		<td class="leftid"><input type="checkbox" id="checkall" onclick="checkList('checkall')"></td>
		<td style="text-transform : uppercase;"><?php echo $spText['common']['Id']?></td>		
	...
	if(count($list) > 0){
		...
		foreach($list as $i => $listInfo){
			...
			<tr class="<?php echo $class?>">
				...
				<?php if(!empty($isAdmin)){ ?>
					<td class="td_br_right"><?php echo $listInfo['username']?></td>
				<?php } ?>
				...
				<td class="td_br_right"><?php echo $listInfo['last_updated']?></td>
				...
				<td class="<?php echo $rightBotClass?>">
					...
					?>
					<select name="action" id="action<?php echo $listInfo['id']?>" onchange="doAction('siteauditor.php', 'content', 'project_id=<?php echo $listInfo['id']?>&pageno=<?php echo $pageNo?>', 'action<?php echo $listInfo['id']?>')">
